...
1. **Solo biblioteca estándar de Python.** Permitido: `sqlite3`, `argparse`, `datetime`, `pathlib`, `os`, `sys`. Prohibido añadir dependencias externas en Fase 1.
2. **La fuente de verdad es SQLite**, ubicada en `WORKLOG_HOME` o `~/.worklog/worklog.db`. Nunca escribas estado en el proyecto salvo con `export`.
...
4. **El esquema se auto-crea** si la BD no existe. Operaciones idempotentes.
5. **El proyecto se identifica por la ruta absoluta** del directorio actual de ejecución.
